...
class Son_thread_data_rec(QThread):
    ...
    def audio_moni(self):
        with open('AudioClassification/audio_record.txt', 'w') as file:  #会覆盖
            str1 = str(random.uniform(0, 0.2))  #获取一个12~33之间的浮点数)
            str2 = str(random.uniform(0, 1))
            str3 = str(random.uniform(0, 0.5))
            file.write(str1 + ',' + str2 + ',' + str3)
        
        
        
        
    def status_moni(self,num):
        #电脑——>离开——>书籍——>手机——>离开
        with open('SVMImageClassification/classification_results.txt', 'a') as file:  #追加指令
            str1 = 'SVMImageClassification/test_photo/output_image_1.png:'
            list_num=[0,0,0,3,2,1,1,0,0,1,2]
            str2 = str(list_num[num])
